...
# CNN(Convolution Nerual Network) 모델의 구조 만들기
X = tf.keras.layers.Input(shape=[28, 28, 1])

H = tf.keras.layers.Conv2D(6, kernel_size=5, padding='same', activation='swish')(X)  # 3개의 특징맵 =  3채널의 특징맵
H = tf.keras.layers.MaxPool2D()(H)
H = tf.keras.layers.Conv2D(16, kernel_size=5, activation='swish')(H)  # 6개의 특징맵 =  6채널의 특징맵
H = tf.keras.layers.MaxPool2D()(H)
...
# ----------------------------------------------------------------------
# 데이터 준비하기
(indep, dep), _ = tf.keras.datasets.cifar10.load_data()
print(indep.shape, dep.shape)

dep = pd.get_dummies(dep.reshape(50000))  # 원핫인코딩(표 형태이므로 reshape 한 후)
print(indep.shape, dep.shape)
